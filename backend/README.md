...
개인용 웹이 호출하는 창구는 `/api/public` 아래에 모여 있습니다.
**호출 주체는 개인용 서버를 권합니다** — 브라우저가 직접 부르면 CORS 설정이 필요하고
이 API를 인터넷에 노출해야 합니다.

| 순서 | 경로 | 하는 일 |
| --- | --- | --- |
| 1 | `GET /api/public/programs` | 사업 목록. `contract_ready`가 false면 아직 서식이 없는 사업 |
| 2 | `GET /api/public/programs/{id}/contract-form` | **챗봇이 물어볼 항목 목록** |
| 2 | `GET /api/public/programs/{id}/contract-form?donation_type=legacy` | 유산기부 서식. `legacy_ready`가 false면 그 사업은 유산기부를 받지 않음 |
| 3 | `POST /api/public/agreements` | 모은 값 → PDF 생성 → 서명 요청 메일 발송 |
| 4 | `GET /api/public/agreements/{id}` | 서명 완료 여부 조회 |
| 5 | `POST /api/public/legacy/recordings` | **유산기부 녹음유언이 만들어졌다는 사실** 수신 |

**2와 3이 짝입니다.** 2번이 알려준 `field.key`를 3번의 `values` 키로 그대로 쓰면
됩니다. 기관이 서식을 바꾸면 2번 결과가 바뀌고 챗봇 질문도 저절로 따라갑니다.

### 유산기부 녹음유언 통보 (5번)

기업용은 **서명만 한 사람과 녹음유언까지 마친 사람을 구분해야** 합니다. 둘은 법적으로
다릅니다 — 녹음이 있어야 민법 제1067조 녹음유언 요건을 논할 수 있고, 서명만 있으면
기부 의사 표시일 뿐입니다. 사후 집행 준비·가족 안내·검인 대응이 이 차이에 달려 있습니다.

개인용 웹이 같은 건에 대해 최대 두 번 보냅니다.

| 시점 | `status` |
| --- | --- |
| 녹음이 저장된 직후 | `recorded` |
| 자가 확인 5개를 모두 마쳤을 때 | `verified` |

실패하면 개인용이 나중에 다시 보냅니다. **멱등하게 처리합니다** — 파일 지문이 같으면
같은 녹음으로 보고, 늦게 도착한 `recorded`가 이미 끝난 `verified`를 되돌리지 않습니다.
다시 녹음하면 지문이 달라지므로 그때는 새 녹음으로 받습니다.

```json
{
  "agreement_id": "agr_0003", "pledge_id": "lg_a7eaf...", "program_id": "prog_4",
  "status": "verified", "recorded_at": "2026-08-02T03:54:06.684Z",
  "duration_ms": 5000, "sha256": "d28fe05a...", "has_witness": true,
  "checklist_passed": 5, "checklist_total": 5, "spec_version": "2026-07-31.1"
}
```

응답: 없는 `agreement_id` → 404, 알 수 없는 `status`나 유산이 아닌 약정 → 400.

**받지 않는 것 — 요청하지 마세요.** 녹음 파일, 대본 전문, 유언 내용, 증인 이름·연락처.
증인은 있었는지(`has_witness`)만 받습니다. **생전에 기관이 유언 내용을 열람하면 부당한
영향력 행사 의혹의 빌미가 됩니다.** 그래서 W4 상세에도 **재생 버튼이 없습니다.**
사후 집행 시점에 필요하면 그때 기부자 쪽에서 받는 구조입니다.

화면에는 이렇게 나옵니다.

- **W2 약정 목록** — 유산 약정 행에 `녹음 확인 완료` / `녹음 완료` / `서명만` 배지
- **W4 약정 상세** — 녹음 시각·길이·파일 지문(SHA-256)·증인 등록 여부·자가 확인 n/5
- **W3 관리 필요** — `녹음유언 미완료` 규칙. 서명 후 N일(기본 14일) 지나도 녹음이 없으면
  담당자가 연락할 대상으로 잡힙니다. `data/risk_rules.json`에서 조정합니다.

금액 집계·달성률에는 넣지 않습니다. 유산 약정은 `amount`가 비고 `donation_type: "유산"`만
들어가며, **건수만** 셉니다(유산 약정 화면 참조).

### 기관 선입력 항목

2번 응답은 항목을 두 갈래로 나눠 줍니다.

| 필드 | 뜻 |
| --- | --- |
| `prefilled` | 기관이 서식에 미리 채워 둔 값 (`{"org_name": "부산대학교", ...}`) |
| `fields` | 기부자가 채워야 하는 나머지 — **챗봇은 이것만 물어봅니다** |

후원기관명·담당 부서처럼 기관이 정하는 값은 W6에서 미리 적어 둡니다.
그러면 `prefilled`로 내려가고 질문 목록에서 빠집니다.

3번을 부를 때 `prefilled` 값은 **보내지 않아도 됩니다.** 서버가 서식에서 다시
읽어 합칩니다. 보내더라도 **기관이 정한 값이 이깁니다** — 기부자가 후원기관명을
바꿔 보내도 서식의 값으로 덮어씁니다.
...
{
  "program_id": "prog_1",
  "values": { "donor_name": "...", "amount": "30000", "frequency": "월", ... },
  "signer": { "name": "...", "email": "..." }
}
```

값이 빠져 있으면 모두싸인을 부르기 전에 400과 함께 빠진 항목 이름을 돌려줍니다.
챗봇이 다시 물어보면 됩니다.

`schema_version`은 항목 구성이 바뀌면 값이 달라집니다. 대화 시작 시 기억해 두었다가
3번을 부르기 전에 비교하면, 기관이 중간에 서식을 바꾼 걸 알아챌 수 있습니다.

### 발송된 약정은 자동으로 화면에 나타납니다

3번으로 발송한 문서는 모두싸인에 남고, 기업용 웹은 화면을 그릴 때마다 모두싸인을
읽습니다. 그래서 **W1 대시보드·W2 기부 현황·W4 약정 상세에 따로 만들 것 없이 나타납니다.**

단, `metadatas`에 집계 키가 들어가야 금액이 잡힙니다. 이건 서버가 `values`에서
자동으로 뽑아 넣습니다(`services/agreements.py`의 `_metadatas`).

---

## 계약서 PDF

`services/contract.py`가 약정서를 새로 그립니다. 스캔한 서식은 **어떤 항목을 받는지
알아내는 용도**이고, 최종 문서는 여기서 만듭니다.

**`(서명)` 앵커가 핵심입니다.** 모두싸인은 PDF 본문에서 이 글자를 찾아 옆에 서명란을
놓습니다. 이 글자를 지우면 서명란 없는 문서가 발송됩니다.

한글은 `assets/fonts/NanumGothic-Regular.ttf`로 렌더링합니다(OFL 라이선스, 재배포 가능).
이 파일이 없으면 한글이 깨집니다. 원문자(①②③)는 이 폰트에 글리프가 없어 쓰지 않습니다.
...
모두싸인이 호출하려면 **공인 URL**이 필요해 `localhost`로는 등록할 수 없습니다.
터널을 열어 로컬 서버를 잠시 인터넷에 노출시킵니다.

```bash
ngrok http 8080
```

실행하면 `Forwarding https://<무작위>.ngrok-free.app -> http://localhost:8080` 이
찍힙니다. 그 https 주소가 터널 주소입니다.

1. 모두싸인 웹 → **설정 → API → Webhook** 에 아래 주소를 등록합니다.
   ```
   https://<터널주소>/api/webhooks/modusign
   ```
2. 개인용 웹에서 약정을 하나 보내고 **서명을 끝냅니다.**
3. 서버 로그에 이 줄이 찍히면 수신된 것입니다.
   ```
   웹훅 반영: agr_0004 → COMPLETED (document_all_signed)
   ```
   약정 상태가 폴링을 기다리지 않고 즉시 바뀝니다.

> ⚠️ **ngrok 무료 주소는 실행할 때마다 바뀝니다.** 다시 켜면 모두싸인에 새 주소로
> 다시 등록해야 합니다. 시연 직전에 한 번 확인하세요.

> 터널이 불안정할 수 있으니 발표 때는 **웹훅 수신 로그 캡처**를 백업으로 두고,
> "웹훅이 없어도 폴링으로 동작한다"는 이중화를 함께 설명하는 편이 안전합니다.
> 웹훅 없이도 개인용 웹이 10초 폴링으로 상태를 따라갑니다.

**폴링 부하.** 웹훅으로 갱신된 지 2분 안의 끝난 문서는 `GET /api/public/agreements/{id}`가
모두싸인을 다시 부르지 않고 저장된 상태를 바로 돌려줍니다
(`services/agreements.py`의 `WEBHOOK_FRESH_SECONDS`). 진행 중인 문서는 그대로 확인합니다.
